...
const navLinks = [
  ...
  },
  {
    to: "projects",
    label: "Projetos",
    description: "Projetos desenvolvidos para aprendizado",
    icon: FolderOpenDot,
  },
  {
    to: "experience",
  ...
  },
  {
    to: "contact",
    label: "Contato",
    description: "Entre em contato e vamos evoluir juntos",
    icon: Contact,
  },
];
    ...
    <>
      <header className="fixed top-0 left-1/2 transform -translate-x-1/2 z-50 flex items-center justify-between w-full max-w-5xl py-8 transition-colors px-10">
          ...
          {navLinks.map(({ to, label }) => (
            <Link
              ...
              className={`cursor-pointer transition-colors hover:text-link-hover-color ${
                location.pathname.includes(to) ? "text-link-hover-color" : ""
              }`}
            >
              {label}
            </Link>
        ...
        <div className="flex items-center gap-3 relative z-50">
          <button
            ...
            className="md:hidden flex flex-col justify-center items-center w-10 h-10 gap-[6px] rounded-full border border-border-color bg-contrast-color transition-colors dark:bg-contrast-color-dark dark:border-border-color-dark"
          >
            <span
              className={`block h-[2px] w-5 bg-current text-primary-color dark:text-primary-color-dark rounded-full transition-all duration-300 origin-center ${
                open ? "rotate-45 translate-y-[8px]" : ""
              }`}
            />
            <span
              className={`block h-[2px] w-5 bg-current text-primary-color dark:text-primary-color-dark rounded-full transition-all duration-300 ${
      ...
      <div
        className={`fixed inset-0 z-40 md:hidden flex flex-col transition-all duration-500 ease-in-out
          bg-[#f9f9f9] dark:bg-[#0f0f0f]
          ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      >
        <div className="flex flex-col justify-center flex-1 px-10 pb-10 pt-28">
          <p className="text-xs font-mono uppercase tracking-widest text-second-color dark:text-second-color-dark opacity-40 mb-10">
            — navegação
          </p>

          <nav className="flex flex-col gap-2">
            {navLinks.map(({ to, label, description, icon: Icon }, index) => (
              <Link
                key={to}
                to={to}
                className={`group flex items-center gap-5 py-4 border-b border-border-color dark:border-border-color-dark
              ...
              >
                <div className="p-2.5 rounded-xl border border-border-color bg-contrast-color dark:bg-contrast-color-dark dark:border-border-color-dark transition-colors group-hover:border-link-hover-color/40">
                  <Icon
                    size={20}
                ...
                <div className="flex-1">
                  <p
                    className={`font-semibold text-lg transition-colors group-hover:text-link-hover-color ${
                      location.pathname.includes(to)
                        ? "text-link-hover-color"
                        : "text-primary-color dark:text-primary-color-dark"
                    }`}
                  ...
                  <p className="text-xs text-second-color dark:text-second-color-dark opacity-60 mt-0.5">
                    {description}
                  </p>
                </div>

                <span className="text-second-color opacity-30 group-hover:opacity-100 group-hover:text-link-hover-color transition-all duration-200 text-lg">
                  →
                </span>
              </Link>
            ))}
